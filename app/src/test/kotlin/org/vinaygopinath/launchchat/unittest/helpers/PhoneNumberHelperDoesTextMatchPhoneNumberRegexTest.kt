...
class PhoneNumberHelperDoesTextMatchPhoneNumberRegexTest {
    ...
    fun `returns true when text is phone number with no formatting and no country code`() {
        ...
        assertThat(helper.doesTextMatchPhoneNumberRegex(inputText)).isTrue()
    }
    ...
    fun `returns false when text includes non-numerical and non-formatting characters`() {
        ...
        assertThat(helper.doesTextMatchPhoneNumberRegex(inputText)).isFalse()
    }
}
